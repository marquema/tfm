...
Este módulo define el esquema persistente que conecta todas las fases del
pipeline (screener → preparación de datos → entrenamiento → simulación)
y permite que la API, el dashboard y el frontend consulten información
coherente sobre qué se entrenó, con qué datos y qué resultados produjo.
...
Arquitectura desacoplada del motor de BD:
  ...
  - `init_db()` crea las tablas que falten y aplica migraciones ligeras
    (ALTER TABLE ADD COLUMN) para columnas nuevas sin necesidad de Alembic.

Tablas:
  - users: credenciales y rol (admin / investor) para autenticación JWT.
  - screener_results:candidatos seleccionados por el screener de mercado con sus métricas (Sharpe rolling, sector, volumen, volatilidad).
                       Solo un resultado activo a la vez; los anteriores quedan como histórico. Actúa como "fuente de verdad" para los
                       tickers que /preparar-datos usa por defecto.
  - universes: snapshot del universo de activos sobre el que se entrenó  y validó un modelo (tickers, rango de fechas, n_features).
        Cada cambio de tickers crea un nuevo Universe, lo que   permite detectar automáticamente modelos incompatibles.
  - trained_models: metadatos del modelo entrenado (tipo, ruta del .zip/.pkl,
       pasos, métricas y perfil de riesgo usado). Vinculado al  Universe con el que fue entrenado para garantizar que la
       simulación se ejecute con datos coherentes.
  - simulations: historial de simulaciones ejecutadas por cada inversor
                       (capital, comisión y resumen de resultados). Permite
                       consultar backtests previos sin tener que recomputarlos.

Relaciones:
  Universe 1──N TrainedModel(un universo puede tener varios modelos entrenados)
  Universe 1──N Simulation (cada simulación opera sobre un universo concreto)
  User 1──N Simulation (cada inversor conserva su historial de simulaciones)

Notas de diseño:
  - Los campos tipo JSON se usan para listas heterogéneas y métricas anidadas
    que evolucionan con el proyecto sin forzar migraciones por cada cambio.
  - `is_active` en ScreenerResult y Universe garantiza que el resto del sistema
    siempre tenga un "registro vigente" sin ambigüedad, sin borrar el histórico.
  - Las fechas se almacenan como texto 'YYYY-MM-DD' (String(10)) para máxima
    portabilidad entre motores y para simplificar la serialización a JSON.
"""
...
engine = create_engine(
    _DB_URL, echo=False,
    # check_same_thread solo en SQLite; PostgreSQL lo ignora
...
class User(Base):
    ...
    # Relación uno a muchos: un usuario tiene muchas simulaciones
...
def init_db():
    ...
    SQLAlchemy solo crea las tablas que faltan, no modifica las existentes. 
    En producción se usaría Alembic para migraciones de esquema.
    """
