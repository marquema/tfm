"""
Gestión de la configuración del universo de activos. [LEGACY — en retirada]

Esta era la primera implementación del "contrato de universo" basada en un
fichero JSON (data/universe_config.json) que actuaba como fuente de verdad
para garantizar la coherencia entre datos preparados, modelos entrenados
y simulaciones.

Estado actual:
  - La fuente de verdad la he movido a la BD (tablas Universe / TrainedModel
    en src/auth/models.py). La validación de compatibilidad ahora la hace
    universe_repository.validate_model_compatibility().
  - Solo `save_config` sigue en uso, escribiendo el JSON en paralelo a la BD
    como fallback retrocompatible — universe_repository._migrate_from_json
    lo lee si la BD está vacía para no perder datos preparados antes de
    introducir el esquema persistente.
  - El resto de funciones (load_config, get_tickers_from_config,
    validate_compatibility, save_model_metadata, load_model_metadata)
    quedan comentadas más abajo: nadie las llama y se conservan solo como
    referencia histórica del diseño previo. Cuando se retire del todo el
    JSON legacy, este módulo desaparecerá entero.
"""

import os
import json
from datetime import datetime
# from typing import Optional  # solo usado por funciones legacy comentadas más abajo
...
# ─────────────────────────────────────────────────────────────────────────────
# DEPRECATED — funciones legacy del contrato basado en JSON.
#
# Sustituidas por la fuente de verdad en BD:
#   - load_config / get_tickers_from_config →
#         universe_repository.get_active_universe(db).tickers
#   - validate_compatibility →
#         universe_repository.validate_model_compatibility(db, model_type)
#   - save_model_metadata / load_model_metadata →
#         tabla TrainedModel (campos universe_id, train_metrics, ...)
#
# Se conservan comentadas como referencia histórica del diseño previo. No
# borrar todavía: ver decisión final cuando se retire el JSON legacy
# (data/universe_config.json) y la migración _migrate_from_json.
# ─────────────────────────────────────────────────────────────────────────────
