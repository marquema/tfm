...
def get_table_div_classic(tickers, start, end_date):
    ...
    if not df.empty:
        ...
        df.to_csv("data/tutor_resultados_dividendos.csv", index=True, encoding="utf-8-sig")
...
def generar_dataset_completo(tickers, start_date, end_date, train_pct=0.8):
    """
    ...
    if 'IBIT_retornos' in dataset.columns and 'IVV_retornos' in dataset.columns:
        ...
        # Antes del lanzamiento del ticker (IBIT pre-2024) la correlación es indefinida → 0
        dataset['corr_IBIT_IVV'] = dataset['corr_IBIT_IVV'].fillna(0)

    # Limpiar y alinear
    # NO usamos dropna(): eliminaría todos los años anteriores al ticker más joven.
    # Estrategia: activos que aún no existían → features = 0 (señal neutra, equivalente a cash),
    #             precios = constante retroalimentado (retorno diario = 0 antes del lanzamiento).
    # Así el agente aprende a no asignar peso a activos sin información real.
    dataset = dataset.ffill().fillna(0)
    precios_originales = precios_originales.bfill().ffill()
    precios_originales = precios_originales.loc[dataset.index]
    ...
    # -----------------------------------------------------------------------
    # Normalización Z-Score SIN leakage temporal
    # Los estadísticos (media, std) se calculan SOLO sobre el conjunto de train
    # (primer train_pct del dataset en orden cronológico) y luego se aplican
    # a todo el dataset. Así el test set nunca "informa" la normalización.
    # -----------------------------------------------------------------------
    dataset = dataset.sort_index(ascending=True)           # orden cronológico
    precios_originales = precios_originales.sort_index(ascending=True)

    n_train    = int(len(dataset) * train_pct)
    mean_train = dataset.iloc[:n_train].mean()
    std_train  = dataset.iloc[:n_train].std().replace(0, 1)  # evitar división por cero

    dataset_norm = (dataset - mean_train) / std_train

    # Guardar estadísticos de normalización (necesarios en inferencia en producción)
    os.makedirs("data", exist_ok=True)
    norm_stats = pd.DataFrame({'mean': mean_train, 'std': std_train})
    norm_stats.to_csv("data/norm_stats.csv", encoding="utf-8-sig")

    # Guardar en orden ASCENDENTE (cronológico) — correcto para el entorno Gymnasium:
    # índice 0 = dato más antiguo, índice N = dato más reciente
    # El split train/test (start_idx/end_idx) queda así como oldest 80% / newest 20%
    precios_originales.to_csv("data/precios_originales.csv", encoding="utf-8-sig")
    dataset_norm.to_csv("data/features_normalizadas.csv", encoding="utf-8-sig")
    ...
    print(f"Normalización: stats calculadas sobre los {n_train} días de train ({train_pct*100:.0f}%).")
    return dataset_norm, precios_originales
